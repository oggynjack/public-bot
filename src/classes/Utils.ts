...
export class Utils {
    ...
    public async paginate(
        ...
        if (ctx instanceof ChatInputCommandInteraction) {
            if (ctx.deferred) {
                await ctx.followUp({ ...msgOptions });
            } else {
                await ctx.reply({ ...msgOptions });
            }
            sentMsg = await ctx.fetchReply();
        } else {
            sentMsg = await (ctx.channel as TextChannel).send({ ...msgOptions });
        }

        const author = ctx instanceof CommandInteraction ? ctx.user : (ctx as Message).author;
        ...
        collector.on("end", async () => {
            await sentMsg.edit({ embeds: [embed[page]], components: [] });
        });
    }
